...
/*
$result2 = pg_query($connect, "select isnull(max(dt)+1,getdate()-479)::date maxdt, dateadd(day,1,isnull(max(dt)+1,getdate()-479))::date from public.bc_source_type");
...
drop table if exists public.bc_source_type_staging;
CREATE TABLE public.bc_source_type_staging  ( 
...
copy bc_source_type_staging
from 's3://$S3bucketName/bcoutput_source_type.json' with 
credentials 'aws_access_key_id=$S3accessKey;aws_secret_access_key=$S3secretKey' 
json  'auto'
$S3Region;

/* Get rid of bad data */
delete from bc_source_type_staging where video_view is null;

/* Update Date */
update bc_source_type_staging set dt = '$fromdate';




/*  Delete existing data so that we can load clean data*/
delete from public.bc_source_type
where exists 
(select 1 from public.bc_source_type_staging b where public.bc_source_type.dt=b.dt );


/* Load the final de-duped data */
insert into public.bc_source_type
select distinct * from public.bc_source_type_staging a
where not exists (select 1 from public.bc_source_type b where a.dt=b.dt);
