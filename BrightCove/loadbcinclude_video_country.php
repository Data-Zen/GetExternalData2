...
/*
$result2 = pg_query($connect, "select isnull(max(dt)+1,getdate()-479)::date maxdt, dateadd(day,1,isnull(max(dt)+1,getdate()-479))::date from public.bc_videos_country");
...
CREATE TABLE if not exists public.bc_videos_country  ( 
...
drop table if exists public.bc_videos_country_staging;
CREATE TABLE public.bc_videos_country_staging  ( 
	video               	int8 NULL ENCODE LZO ,
	video_view          	int4 NULL ENCODE LZO,
	videoname           	varchar(10000) NULL ENCODE LZO,	
	video_seconds_viewed	int8 NULL ENCODE LZO,
...
copy public.bc_videos_country_staging
...
/* Get rid of bad data */
delete from bc_videos_country_staging where video is null;

/* Update Date */
update bc_videos_country_staging set dt = '$fromdate';
